...
def get_query(param):
    ...
    output = pd.DataFrame.from_records(iter(cur), columns=[x[0] for x in cur.description])
    return output
...
‘Views’""".replace("‘","").replace("’","").split('\n')
    ...
    with st.sidebar:
        ...
        st.markdown(f'''<div style="line-height: 15em;"><span style="text-align: justify;   line-height: 1.2em; vertical-align: bottom;">Built with streamlit {st.__version__}\n<span>
         <p>No Snowpark was harmed while building this App.</p></div>''', unsafe_allow_html=True)
